...
inputs:
    ...
    arg_3: minimum global thresholding value. Must be an integer.
    ...
    Example:
        >>>python W03_assignment.py .../W03_Assignment/NIFTI/head.nii gaussian 0 100 close 2
...
over top of it. Dilation and/or erosion can be applied to the segmentation.
The code will also write a new filtered image as NIFTI file 
...
# Output: an image with binary values representing the segmented areas of the original image 
# 1 value = segmented tissue, 0 value = not segmented
# =============================================================================
...
def imageManipulation(iterations, manipulation):
    ...
    while num > 0:      
        # initiliase the global segmentation image to work with
        inputImage = segmentationImage.GetOutput()
        
        # create a copy of the segmentation image to manipulate
...
#        for z in range(imageDim[2]):
            for y in range(imageDim[1]):
                for x in range(imageDim[0]):
                    ...
                    # determine what operation (or manipulation) to implement
                    if manipulation == 'dilation':
